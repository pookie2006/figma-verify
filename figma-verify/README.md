...
The studio caches a fetched design (by file key + node id) for 5 minutes per run, so re-comparing the *same* Figma link only re-fetches once — this alone should make the error rare unless you're also switching between several different Figma links in quick succession, or the file is on a Starter plan. `fetchFigmaNode` retries a 429 **only when Figma's response includes a `Retry-After` header** — that's the only signal that distinguishes a short per-minute throttle from an exhausted quota that resets monthly; without it, retrying blindly would just spend more of a scarce monthly budget on one failed click, so it fails fast instead. The resulting error message echoes Figma's own `X-Figma-Plan-Tier` / `X-Figma-Rate-Limit-Type` headers when present, so you can see exactly which limit you hit.

**"Is my new token actually being used?"** The studio resolves whatever `FIGMA_TOKEN` is currently set to a real account via Figma's `GET /v1/me` (a separate, high-volume endpoint — doesn't touch the file-fetching budget) and prints it on startup:

```
Figma token: you@example.com (@yourhandle) — this is the account being used for Figma-link compares.
```

It's also on the Figma inserter's hover tooltip in the report, and in `/api/health`'s `figmaIdentity` field. Directly confirms which account is wired up instead of trusting an `export` in some other terminal tab.
